...
bool DistributedStates::check_split(const DistributedStates& dst_distributed_states) const {
  ...
  for (auto o : dst_order) {
    if (o >= 0) {
      if (dst_states.at(o) < states(o) || dst_states.at(o) % states(o) != 0) {
        return false;
      } else {
...
bool DistributedStates::check_allgather(const DistributedStates& dst_distributed_states) const {
  ...
  return states(gather_dim) > 1 && dst_distributed_states.states(-1) > 1 &&
         dst_distributed_states.states(-1) * dst_distributed_states.states(gather_dim) == states(gather_dim);
}
