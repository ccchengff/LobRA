...
void LinearOpImpl::DoDeduceStates(const TensorList& inputs, TensorList& outputs, 
  ...
  if (new_lorder != new_rorder) {
    ...
    // HT_LOG_INFO << "lorder = " << lorder << ", rorder = " << rorder << ", new_lorder = " << new_lorder << ", new_rorder = " << new_rorder;
    // HT_ASSERT(new_lorder == new_rorder) << "new_lorder is not equal to new_rorder! " << new_lorder << " v.s. " << new_rorder;
