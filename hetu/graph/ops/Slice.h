...
class SliceOpImpl final : public ViewsOpImpl {
 public:
  ...
  HTShape get_task_output_shape(const HTShapeList& input_shapes) const {
    HTShape output_shape = get_output_shape();
    if (input_shapes.size() == 1) {
      return output_shape;
    } else {
      int len = output_shape.size();
      for (int i = 0; i < len; i++) {
        if (output_shape[i] == -1) {
          output_shape[i] = input_shapes[0][i] / input_shapes[1][2] * input_shapes[1][1];
        }
      }
      return output_shape;
    }
  ...
  HTShape get_task_begin_pos(const HTShapeList& input_shapes) const {
    ...
    if (input_shapes.size() == 1) {
      ...
      for (int i = 0; i < len; i++) {
        if (begin_pos[i] == -1) {
          begin_pos[i] = input_shapes[0][i] / input_shapes[1][2] * input_shapes[1][0];
        }
      }
      return begin_pos;
    }
 ...
 protected:
  ...
  DoInferMeta(const TensorList& inputs) const override {
    ...
    for (int i = 0; i < len; ++i) {
      HT_ASSERT(_begin_pos[i]->get_val() + output_shape[i] <= inputs[0]->shape(i))
        << "dim " << i << " begin pos is " << _begin_pos[i]->get_val() << " and output len is " << output_shape[i]
...
Tensor MakeSliceOp(Tensor input, Tensor task_batch_idx, const HTShape& begin_pos,
                   const HTShape& output_shape, OpMeta op_meta = OpMeta());

Tensor MakeSliceOp(Tensor input, Tensor task_batch_idx, const SyShape& begin_pos,
                   const SyShape& output_shape, OpMeta op_meta = OpMeta());
 ...
 public:
  ...
  HTShape get_task_output_shape(const HTShapeList& input_shapes) const {
    ...
    if (input_shapes.size() == 2) {
      ...
      for (int i = 0; i < len; i++) {
        if (output_shape[i] == -1) {
          output_shape[i] = input_shapes[1][i] / input_shapes[2][2] * input_shapes[2][1];
        }
      }
      return output_shape;
    }
  ...
  HTShape get_task_begin_pos(const HTShapeList& input_shapes) const {
    ...
    if (input_shapes.size() == 2) {
      ...
      for (int i = 0; i < len; i++) {
        if (begin_pos[i] == -1) {
          begin_pos[i] = input_shapes[1][i] / input_shapes[2][2] * input_shapes[2][0];
        }
      }
      return begin_pos;
    }
