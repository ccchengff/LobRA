...
NDArrayList IndexAddOpImpl::DoCompute(Operator& op,
  ...
  HTShape _start_and_end_idx = inputs.size() == 3 ? HTShape({inputs.at(0)->shape(0) * inputs.at(2)->shape(0) / inputs.at(2)->shape(2),
  ...
  // output_shape[0] = inputs.at(1)->shape(0);
  NDArray slice_x = NDArray::slice(outputs.at(0), {_start_and_end_idx[0], 0}, output_shape, op->instantiation_ctx().stream_index);
  // HT_LOG_INFO << "indexadd op: " << op << "start_and_end_idx = " << _start_and_end_idx << " slice_x shape = " << slice_x->shape() << ", inputs.at(1) shape = " << inputs.at(1)->shape();
...
TensorList IndexAddOpImpl::DoGradient(Operator& op,
  ...
  // HTShape input_start_and_end_idx{0, op->input(0)->shape(0)};
  // HT_LOG_INFO << "input_start_end_idx = " << input_start_and_end_idx << ", grad_output shape = " << grad_outputs.at(0)->shape() << ", input shape = " << op->input(0)->shape();
...
void IndexAddGradientOpImpl::DoCompute(Operator& op,
  ...
  if (!require_slice()) {
    ...
    // HT_LOG_INFO << "start_and_end_idx = " << _start_and_end_idx << ", slice_grad shape = " << slice_grad->shape() << ", input shape = " << inputs.at(0)->shape() << ", output shape = " << outputs.at(0)->shape();
