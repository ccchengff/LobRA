...
class LLamaAttention(ht.nn.Module):
    ...
    def forward(
        ...
        qkv = self.qkv_proj(hidden_states, task_batch_idxs=task_batch_idxs)
...
class LLamaModel(ht.nn.Module):
    ...
    def forward(
        ...
        if token_type_ids is not None:
            if hidden_states.check_multi_ds_equal(self.wte.ds_map['split0_dup']): # pure dp
                inputs_embeds = hidden_states
            else:
                inputs_embeds = ht.comm(hidden_states, self.wte.ds_map['split0_dup'])
            token_type_embeds = self.wte(token_type_ids) # [b, seq_len, embed_dim]
            if token_type_embeds.check_multi_ds_equal(self.wte.ds_map['split0_dup']): # pure dp
                token_type_embeds = token_type_embeds
            else:
                token_type_embeds = ht.comm(token_type_embeds, self.wte.ds_map['split0_dup'])
    ...
    def forward(
        ...
        if labels is not None:
            if self.config.train_task_num == 1:
                ...
                shift_labels_of_tasks = ht.split(shift_labels, task_batch_idxs, dim=0, name=f'split_task_shift_labels')
                shift_lm_logits_of_tasks = ht.split(shift_lm_logits, task_batch_idxs, dim=0, name='split_task_shift_lm_logits')
